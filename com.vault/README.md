...
`com.vault` depends on `com.auth` for secret access authorization. Every secret key lookup is checked before Vault, environment, or config fallback values are returned.
...
- `com.vault.AuthorizedVaultService`: calls `com.auth.secret.SecretAuthorizationService` before delegating secret lookup.
...
- `vault.authorization.application-id`: application/module id allowed to access configured keys.
- `vault.authorization.allowed-keys`: comma-separated list of allowed secret keys; `*` allows all keys for that application.
