...
`map.cgmes.iidm` contains the CGMES-to-IIDM and IIDM-to-CGMES transformation layer. It follows the same separation visible in GridSuite-style architectures: shared data contracts, reusable services, and dedicated mapping/adaptation modules are kept separate from application services.
...
```bash
mvn -Dmaven.repo.local=work/m2 -pl map.cgmes.iidm -am test
```
